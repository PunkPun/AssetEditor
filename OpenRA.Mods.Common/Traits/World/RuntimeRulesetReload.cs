...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class RuntimeRulesetReloadCommandInfo : TraitInfo
	{
		public override object Create(ActorInitializer init)
		{
			return new RuntimeRulesetReloadCommand();
		}
	}

	public class RuntimeRulesetReloadCommand : IChatCommand, IWorldLoaded, IPostWorldLoaded
	{
		...
		void IPostWorldLoaded.PostWorldLoaded(World w, WorldRenderer wr)
		{
			if (Enabled && world.LobbyInfo.GlobalSettings.GameSavesEnabled)
			{
				...
				rulesetWatcher = new RulesetWatcher(w, Game.ModData);
			}
		}
		...
		void IChatCommand.InvokeCommand(string name, string arg)
		{
			...
			if (name == CommandName)
			{
				...
				{
					rulesetWatcher?.Dispose();
					rulesetWatcher = new RulesetWatcher(world, Game.ModData);
				}
				else
				{
					rulesetWatcher?.Dispose();
